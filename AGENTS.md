# Agent Instructions

You will always:
- Use emojis instead of SVG icons when appropriate for a quick, playful feel, but prioritize `lucide-react` for professional UI components as per standard system instructions.
- Do not change model strings found in code (e.g., specific ID formats or constant naming conventions).
- Avoid using complex gradients; prefer clean, flat, or subtly shaded modern designs.
- Ensure all ERP data logic follows the "First Recv and Last Delivery" log preservation rule.
- Maintain the "Next Plan" clear logic where data is archived in the Buyer Data Bank instead of deleted.
